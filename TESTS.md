...
| `constraint_persona_daemon_hands_over_between_copied_spirit_databases` | Starts real current and next `persona-spirit` daemons, records through current, copies the current store to next, drives `AttemptHandover` through the real `persona-daemon`, and proves next serves the copied state after the handover while current closes its ordinary socket. |
