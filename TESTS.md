...
| `constraint_persona_daemon_handover_uses_real_spirit_daemon_binaries` | Starts the real `persona-daemon` binary and two real `persona-spirit-daemon` binaries, records through current, copies the store to next, drives owner `AttemptHandover`, and proves the `ActiveVersionChanged` event plus active-version snapshot persist with commit sequence `1`. |
