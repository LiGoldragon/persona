...
| `constraint_persona_engine_drives_version_handover_over_component_upgrade_socket` | Binds a Unix private upgrade socket as a fake current component daemon, decodes the real `signal-version-handover` frames emitted by the Persona manager, replies with typed handover frames, and proves the manager records the active-version change from the returned marker. |
| `constraint_owner_attempt_handover_drives_component_upgrade_socket` | Sends owner `AttemptHandover` to the manager, proves it drives the same private upgrade socket protocol, and persists the marker-backed active selector. |
...
| `constraint_persona_daemon_owner_socket_drives_version_handover` | Starts the real `persona-daemon`, sends `AttemptHandover` over the daemon owner socket, serves a fake component private upgrade socket, and verifies the active selector persists in `manager.redb`. |
...
| `constraint_persona_daemon_owner_socket_drives_real_spirit_upgrade_socket` | Starts the real `persona-daemon`, starts a real `persona-spirit` daemon upgrade socket, sends owner `AttemptHandover`, and proves the manager receives `HandoverMarker` / `HandoverAccepted` / `HandoverFinalized` through the actual Spirit upgrade boundary. |
