...
| `constraint_component_unit_manager_dispatches_start_stop_restart_status` | Starts the `ComponentUnitManager` actor with a recording controller and proves start, stop, restart, and status messages dispatch through the typed unit-control boundary. |
| `constraint_transient_unit_definition_projects_exec_start_and_environment` | Builds a transient unit definition from a component command and proves the model projects systemd `ExecStart` path, argv, restart policy, description, and environment without requiring host systemd. |
| `constraint_persona_engine_refuses_handover_when_next_marker_is_stale` | Binds fake main and next private upgrade sockets with mismatched commit sequences and proves Persona refuses before main enters readiness or the active selector changes. |
| `constraint_persona_engine_recovers_current_handover_when_completion_fails` | Binds fake main and next private upgrade sockets, makes main accept readiness but reject completion, and proves Persona sends `RecoverFromFailure` while leaving the active selector unchanged. |
| `constraint_meta_attempt_handover_drives_component_upgrade_socket` | Sends meta `AttemptHandover` to the manager, proves it drives the same two-socket private upgrade protocol, and persists the marker-backed active selector. |
| `constraint_engine_manager_refuses_handover_with_quarantined_version` | Records a meta `Quarantine` event, then proves normal handover refuses that component version before opening any private upgrade socket. |
| `constraint_meta_attempt_handover_reports_quarantined_version` | Records a meta `Quarantine` event, then proves meta `AttemptHandover` returns a typed `Rejected(VersionQuarantined)` reply instead of closing the meta socket. |
| `constraint_persona_daemon_meta_socket_drives_version_handover` | Starts the real `persona-daemon`, sends `AttemptHandover` over the daemon meta socket, serves fake main and next component private upgrade sockets, and verifies the active selector persists in `manager.sema`. |
| `constraint_persona_daemon_meta_handover_uses_injected_unit_controller` | Starts an in-process `PersonaDaemon` with a recording unit controller, sends meta `AttemptHandover`, and proves the daemon path starts the next version's component unit before driving handover sockets. |
...
| `constraint_persona_daemon_serves_meta_version_handover_socket` | Starts the real `persona-daemon`, sends `meta-signal-version-handover::Quarantine` through its derived meta socket, and proves the request reaches the manager meta-authority path. |
| `constraint_persona_daemon_meta_socket_drives_real_spirit_upgrade_socket` | Starts the real `persona-daemon`, starts real main and next `persona-spirit` daemon upgrade sockets, sends meta `AttemptHandover`, and proves the manager receives next marker parity plus main `HandoverAccepted` / `HandoverFinalized` through actual Spirit upgrade boundaries. |
...
| `constraint_persona_daemon_handover_uses_real_spirit_daemon_binaries` | Starts the real `persona-daemon` binary and two real `persona-spirit-daemon` binaries, records through current, copies the store to next, drives meta `AttemptHandover`, and proves the `ActiveVersionChanged` event plus active-version snapshot persist with commit sequence `1`. |
| `constraint_persona_daemon_recovers_real_spirit_after_completion_failure` | Starts the real `persona-daemon` binary and two real `persona-spirit-daemon` binaries, forces current active version `v0.1.0`, injects a completion failure after current readiness, proves Persona sends `RecoverFromFailure`, and proves the current Spirit daemon returns to ordinary writes without persisting a `v0.1.1` active selector. |
| `persona-daemon-launches-nix-built-prototype-topology` | Starts the real `persona-daemon` with the Nix-built prototype launcher set, proves all eight prototype-supervised components receive the spawn-envelope environment and point at real component package binaries, proves every domain and supervision socket binds in a pure Nix builder, and proves the manager records readiness only after typed supervision replies. Terminal PTY readiness is deliberately left to the stateful terminal-cell smoke lane. |
